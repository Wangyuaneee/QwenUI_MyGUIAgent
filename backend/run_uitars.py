...
####################################### Edit your Setting #########################################
# Your ADB path
adb_path = os.getenv("ADB_PATH", "C:\\adb\\platform-tools\\adb") 
adb_path = f'"{adb_path}"' if " " in adb_path and not adb_path.startswith('"') else adb_path

# Your instruction
instruction = "打开网易BUFF，帮我看看现在爪子刀什么价格"
...
while True:
    ...
    def double_second_value(match):
        x = int(match.group(1))
        y = int(match.group(2)) * 2340 / 1000  # 第二个值乘2
        return f"({x}, {y})"
    action_pre = re.sub(r"(\d+)\s+(\d+)", double_second_value, action_pre)
